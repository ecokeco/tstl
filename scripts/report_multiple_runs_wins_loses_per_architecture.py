...
# Write table header
tlWins = {"convnetquakeingv": 0, "magnet": 0, "mlstm_fcn": 0, "tcn": 0}
tlLoses = {"convnetquakeingv": 0, "magnet": 0, "mlstm_fcn": 0, "tcn": 0}
for baselineModelName, ignoredDatasets in models:
    for datasetName, taskType in datasets:
        ...
        architecture_name = baselineModelName.split("-")[0]
        
        # Compare them by performance
        if taskType == "regression":
            diff = (nontl_metric_avg - tl_metric_avg) / nontl_metric_avg * 100
            if diff > 0:
                tlWins[architecture_name] += 1
            elif diff < 0:
                tlLoses[architecture_name] += 1
        elif taskType == "classification":
...
for architecture_name in list(tlWins.keys()):
    print(architecture_name)
    print("TL wins: {}".format(tlWins[architecture_name]))
    print("TL loses: {}".format(tlLoses[architecture_name]))
