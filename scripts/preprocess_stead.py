...
keys = list(dataset["earthquake/local/"].keys())
...
n = len(keys)
signalShape = dataset["earthquake/local/" + keys[0]].shape
...
EQ = dataset["earthquake/local/"]
	
# Load training data and scale to range [0, -1]
print("Writing training data")
print("0/" + str(len(train)), end="")
mask = np.lib.format.open_memmap(scriptDir + "/../data/{}_MASK.npy".format(args.name), mode="w+", dtype="float64", shape=signalShape)
X = np.lib.format.open_memmap(scriptDir + "/../data/{}_train_X.npy".format(args.name), mode="w+", dtype="float64", shape=(len(train), signalShape[0], signalShape[1]))
...
for index, key in enumerate(train):
	...
	data = EQ[key]
	magnitude = float(data.attrs["source_magnitude"])
