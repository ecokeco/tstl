    ...
    for targetDataset, taskType in targetDatasets: 
        ...
        for model in models:
            ...
            for run in range(7):
                ...
                table[sourceDataset][targetDataset][model]["nontl"].append(referentPerformance)
                table[sourceDataset][targetDataset][model]["tl"].append(tlPerformance)
                table[sourceDataset][targetDataset][model]["lrmultiplier"].append(tlLrMultiplier)
                
# Write to file
report = open(scriptDir + "../reports/performance_per_model.tex", "w")
report.write("\\begin{table}[]\n")
report.write("\\caption{Average percentual gain in predictive performance across seven reruns of the experiment.}\n")
    ...
    for model in models:
        ...
        for targetDataset, taskType in targetDatasets:
            if sourceDataset in targetDataset:
                ...
                nonTlPerformances = table[sourceDataset][targetDataset][model]["nontl"]
                tlPerformances = table[sourceDataset][targetDataset][model]["tl"]
                lrMultipliers = table[sourceDataset][targetDataset][model]["lrmultiplier"]
                print(lrMultipliers)
                avgNonTl = np.mean(nonTlPerformances)
                avgTl = np.mean(tlPerformances)
